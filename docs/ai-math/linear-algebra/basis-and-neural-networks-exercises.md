...
**A1.** 设线性变换 $T$ 在标准基下的矩阵为 $A = \begin{bmatrix}1&2\\3&4\end{bmatrix}$。求 $T$ 在基 $\mathcal{B} = \{(1,1)^{\mathsf T},\; (1,-1)^{\mathsf T}\}$ 下的矩阵表示 $B$。
...
**A2.** 设 $B = P^{-1}AP$（$P$ 可逆）。证明：

(1) $\text{tr}(B) = \text{tr}(A)$；
(2) $\det(B) = \det(A)$；
(3) 若 $A\mathbf{x} = \lambda\mathbf{x}$（$\mathbf{x} \neq \mathbf{0}$），则 $\lambda$ 也是 $B$ 的特征值，并写出对应特征向量。
...
**A3.** 考虑纯线性两层网络：$\mathbf{y} = W_2(W_1 \mathbf{x})$，$W_1 \in \mathbb{R}^{3 \times 4}$，$W_2 \in \mathbb{R}^{2 \times 3}$。

(1) 该网络等效于什么单矩阵变换 $W_{\text{eff}}$？
(2) $W_{\text{eff}}$ 的秩最大是多少？这说明堆纯线性层的本质限制是什么？
(3) 插入 ReLU 后 $\mathbf{y} = W_2 \cdot \text{ReLU}(W_1\mathbf{x})$，还能写成 $W_{\text{eff}}\mathbf{x}$ 吗？为什么？
...
(2) 手算 $\frac{\partial L}{\partial W_2}$ 和 $\frac{\partial L}{\partial W_1}$（用链式法则）。
...
> x = torch.tensor([1., 1.]).unsqueeze(1)
> loss = 0.5 * (W2 @ (W1 @ x) - 5.).pow(2).sum()
> loss.backward()
> print(W1.grad, W2.grad)  # tensor([[12., 12.]])  tensor([[12.]])
> ```
...
**B1.** 为什么说 Embedding 层本质上是基变换？预训练学到的 Embedding 矩阵的列向量的几何含义是什么？

> **标准答案：**
> Embedding 矩阵 \(W \in \mathbb{R}^{d \times |V|}\) 将一个 one-hot 向量 \(\mathbf{e}_i\)（标准基向量）映射为稠密向量 \(\mathbf{w}_i = W\mathbf{e}_i\)（\(W\) 的第 \(i\) 列）。
> **基变换视角**：
> - 标准基（one-hot）中，每个 token 是彼此正交、距离相等的——完全没有语义信息
> - \(W\) 的 \(|V|\) 个列向量定义了一个从 \(\mathbb{R}^{|V|}\)（one-hot 空间）到 \(\mathbb{R}^d\)（语义空间）的线性映射
> - 学到的 \(W\) 的列是「语义基向量」——在这个基下：
>   - 语义相近的词向量夹角小（内积大）
>   - 语义关系的「方向差」是线性的：\(\mathbf{w}_{\text{国王}} - \mathbf{w}_{\text{男人}} + \mathbf{w}_{\text{女人}} \approx \mathbf{w}_{\text{王后}}\)
>
> **几何含义**：每个列向量是该 token 在 \(d\) 维语义空间中的「坐标」。好的 Embedding 找到了一组基，使得**语义方向在该基下线性可分**。这就是分布式表示（distributed representation）的线性代数本质。
> 从压缩角度看：\(W\) 是一个 \(d \times |V|\) 矩阵（\(d \ll |V|\)）。它的列秩 \(\le d \ll |V|\) → 语义信息是低秩的。这也是为什么 Embedding 可以压缩（如从 \(|V| \times 768\) 降到 \(|V| \times 256\)）——真正独立的「语义方向」远少于词汇量。

---

**B2.** 为什么 Layer Normalization 放在激活函数**前面**（Pre-LN）比**后面**（Post-LN）效果更好？从投影几何角度解释。
...
**B3.** 一个两层 ReLU 网络：$\mathbf{h} = \text{ReLU}(W_1 \mathbf{x} + \mathbf{b}_1)$，$\hat{y} = W_2 \mathbf{h} + b_2$。$W_1 \in \mathbb{R}^{3 \times 2}$。

(1) 第一个隐藏神经元在什么条件下激活？在 $\mathbb{R}^2$ 中对应什么几何对象？
(2) 三个神经元定义的线把平面最多分成几个区域？每区域内 $\hat{y}$ 的形式是什么？
(3) 这如何说明「深度网络 = 分段线性函数」？更深 vs 更宽的区别是什么？
...
> - **更深**：每增加一层，新的超平面与已存在折痕相交，在折痕上产生新折痕 → 分段数随深度**指数增长**。深层窄网络可以用很少神经元产生极多分段 → 高效表达复杂函数。
> - **更宽**：单层只是增加超平面数量 → 分段数随宽度**多项式增长**。宽而浅不如深而窄高效。
> 这就是万能逼近定理的构造性直觉：足够多的线性区域 → 可以以任意精度逼近任何连续函数。

---

**B4.** 以下场景中 PCA 在做一种「基变换」。为每个场景说明原始基、新基、保留哪些维度：

(1) 词嵌入矩阵 $N \times 768$ 降到 256 维。
(2) LoRA 对大模型权重做 $W \approx BA$（$B \in \mathbb{R}^{d \times r}$，$A \in \mathbb{R}^{r \times d}$）。
(3) 图像去噪：784 维图像 PCA 保留 $k$ 个主成分后重建。

> **标准答案：**
>
> **(1)**
> - 原始基：标准基（one-hot），每个 token 是正交的 \(|V|\) 维向量
> - 新基：协方差矩阵 \(X^{\mathsf T}X\) 的前 \(k\) 个特征向量——词向量分布方差最大的 \(k\) 个「语义方向」
> - 保留前 256 个主成分（最大特征值），丢弃其余 → 保留 \(>95\%\) 方差
>
> **(2)**
> - 原始基：权重空间 \(\mathbb{R}^{d \times d}\) 的标准基（每个参数一个独立维度）
> - 新基：\(B\) 的 \(r\) 列是 \(r\) 个重要方向的基向量，\(A\) 的行是 \(r\) 个方向的「坐标」
> - LoRA 不做显式 PCA，但思想同源：假设 \(\Delta W\) 低秩 → 只在 \(r\) 个最重要方向上更新（\(r=8\sim64 \ll d\)），丢弃 \(d-r\) 个方向
>
> **(3)**
> - 原始基：784 个像素的标准基（每个像素一个维度）
> - 新基：协方差矩阵的特征向量 = 特征脸（eigenfaces）——像素之间的相关模式。前几个特征脸是低频大结构，后面的越来越精细（皱纹、纹理）
> - 保留前 \(k\) 个特征脸（\(k \ll 784\)），后 \(784-k\) 个丢弃。**噪声集中在高方差方向？不——噪声集中在低方差方向（小特征值方向）。** 保留大方差方向 = 保留结构 + 丢弃噪声（因为噪声在各方向大致等量，结构的信噪比在大方差方向最高）

---

**B5.** Adam 优化器可理解为对角预处理。从线性代数角度解释：

(1) 为什么 SGD 对所有参数用同一个学习率会在 ill-conditioned 问题上失败？
(2) Adam 的 $v_t$ 在做什么？和牛顿法的 $H^{-1}g$ 相比做了什么近似？

> **标准答案：**
>
> **(1)** SGD：\(\theta_{t+1} = \theta_t - \eta g_t\)。学习率 \(\eta\) 受最大 Hessian 特征值 \(\lambda_{\max}\) 限制（\(\eta < 2/\lambda_{\max}\) 防震荡）。但在 \(\lambda_{\min}\) 方向上，每步衰减因子 \(1 - \eta\lambda_{\min} \approx 1 - \frac{2\lambda_{\min}}{\lambda_{\max}}\)，当 \(\kappa = \lambda_{\max}/\lambda_{\min}\) 很大时，该方向几乎不动。SGD 被「最陡方向」绑架了学习率，在「最平方向」上寸步难行。
>
...
> 这是计算效率和预处理质量之间的经典权衡。Adam 在实践中效果极好，因为大量 ML 优化问题中 Hessian 是对角占优的（参数之间的耦合相对较弱）。

---

**B6.** 残差连接 $h_{l+1} = h_l + F(h_l)$ 为什么能训练深层网络？

(1) 写出反向传播的雅可比矩阵，指出和普通层的区别。
(2) 为什么特征值从 $\varepsilon$ 变成 $1 + \varepsilon$ 就防止了梯度消失？
(3) 堆叠 $L$ 个残差块后，梯度乘积展开式长什么样？
...
> 第二项：每层的独立贡献（一阶）。
> 高阶项：跨层交互。
> **每一层的梯度信号都包含来自所有后续层的直接贡献。** 信息和梯度可以在网络中「无损穿行」。这就是 ResNet-152 和 Transformer 96 层可训练的根本原因。

---

**B7.** 推导 Xavier 初始化和 He 初始化的方差公式。

(1) 对于线性层 $h = Wx$，要使 $\text{Var}(h) = \text{Var}(x)$，推导 $\text{Var}(W)$ 的条件。
(2) 为什么反向传播给出不同的约束？
(3) Xavier 为什么取调和平均？He 为什么改成 $2/n_{\text{in}}$？
...
**B8.** 1D 卷积 $y = w * x$ 可写成 $y = Cx$，其中 $C$ 是 Toeplitz 矩阵。

(1) 写出 $w = (w_0, w_1, w_2)$，输入长 $4$（zero-padding $p=1$）对应的 Toeplitz 矩阵 $C$。
(2) 参数共享如何体现在矩阵结构中？参数量的节省有多大？
(3) 转置卷积等价于什么操作？
...
**B9.** Self-Attention $S = \text{softmax}(QK^{\mathsf T}/\sqrt{d})$，$Q,K \in \mathbb{R}^{n \times d}$。

(1) $S$ 的秩最大是多少？当 $d=64$，$n=512$，这意味着什么？
(2) 为什么实际 Transformer 中 $S$ 常常更低秩（attention collapse）？
(3) Multi-head attention 如何缓解这个问题？

> **标准答案：**
>
> **(1)** \(QK^{\mathsf T}\) 的秩 \(\le \min(\text{rank}(Q), \text{rank}(K)) \le \min(n, d) = d\)。Softmax 是逐行操作（指数 + 归一化），不改变秩。所以 \(\text{rank}(S) \le 64\)。
> \(S\) 是 \(512 \times 512\)，但内蕴维度仅 \(\le 64\)——它是**极度低秩**的。相当于用 \(64\) 个「注意力模式」去近似 \(512\) 个 token 的所有 pairwise 关系。
>
> **(2) Attention Collapse**：实际秩常远低于 \(64\)，因为：
> - token 的 query/key 向量高度相关 → \(Q\) 和 \(K\) 自身的有效秩 \(< d\)
> - 很多 token 关注相同的 context（如所有 token 都高度关注 CLS token 或分隔符）→ attention 行向量趋于相同
...
**B10.** 线性回归 $y \approx X\beta$ 的正规方程为 $X^{\mathsf T}X\beta = X^{\mathsf T}y$。

(1) $X^{\mathsf T}X$ 的秩和 $X$ 的秩有什么关系？什么情况下 $X^{\mathsf T}X$ 奇异？
(2) 当 $X^{\mathsf T}X$ 奇异时，正规方程有无穷多解。为什么实际中往往还加 L2 正则？
(3) 从条件数角度证明：$(X^{\mathsf T}X + \lambda I)^{-1}$ 比 $(X^{\mathsf T}X)^{-1}$ 更稳定。

> **标准答案：**
>
> **(1)** \(\text{rank}(X^{\mathsf T}X) = \text{rank}(X)\)。证明：\(\text{rank}(X^{\mathsf T}X) \le \text{rank}(X)\)（显然），且 \(X^{\mathsf T}X\mathbf{v} = 0 \Rightarrow \mathbf{v}^{\mathsf T}X^{\mathsf T}X\mathbf{v} = \|X\mathbf{v}\|^2 = 0 \Rightarrow X\mathbf{v} = 0\)，所以零空间相同 → 秩相同。
...
[← 返回教程](basis-and-neural-networks.md)　　　[← 习题总览](exercises.md)