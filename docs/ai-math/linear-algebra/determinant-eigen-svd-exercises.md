...
**A1.** 化二次型 $f(x_1,x_2)=3x_1^2+2x_1x_2+3x_2^2$ 为标准形，并判断其正定性。
...
**A2.** 求 $A = \begin{bmatrix}3&1\\1&3\end{bmatrix}$ 的特征值与特征向量，写出对角化形式 $A = Q\Lambda Q^{\mathsf T}$。
...
**A3.** 用矩阵微积分求梯度。设 $L(W) = \frac{1}{2}\|WX - Y\|_F^2$，其中 $W \in \mathbb{R}^{d \times n}$，$X \in \mathbb{R}^{n \times m}$，$Y \in \mathbb{R}^{d \times m}$。求 $\nabla_W L$。
...
**A4.** 求 softmax 函数 $\mathbf{s}(\mathbf{z})_i = \frac{e^{z_i}}{\sum_j e^{z_j}}$ 的雅可比矩阵 $J = \partial \mathbf{s} / \partial \mathbf{z}$。
...
**A5.** 不直接计算行列式，仅用行列式的性质判断以下命题的真伪：

(1) $A$ 某一行全 $0 \Rightarrow \det(A) = 0$。
(2) $A$ 两行成比例 $\Rightarrow \det(A) = 0$。
(3) $\det(A)=2 \Rightarrow \det(A^{-1}) = -2$。
(4) 初等行变换 $R_2 \leftarrow R_2 - 3R_1$ 使行列式乘以 $-3$。
(5) $\det(A - \lambda I)=0 \iff \lambda$ 是 $A$ 的特征值。

> **标准答案：**
>
> **(1) 对。** 按该行做 Laplace 展开：每项都含 \(0\) 因子 → 和为 \(0\)。几何上：该行对应的维度被完全压扁 → 体积为 \(0\)。
>
> **(2) 对。** 两行成比例 → 行线性相关 → 高斯消元产生零行 → 行列式为 \(0\)。几何上：两个「边」平行 → 平行多面体退化 → 体积为 \(0\)。
>
> **(3) 错。** \(\det(A^{-1}) = 1/\det(A) = 1/2\)，不是 \(-2\)。行列式乘积法则：\(\det(A)\det(A^{-1}) = \det(AA^{-1}) = \det(I) = 1\)。
>
> **(4) 错。** 把一行的倍数加到另一行**不改变行列式**。验证：\(\det\begin{bmatrix}a&b\\c+3a&d+3b\end{bmatrix} = a(d+3b) - b(c+3a) = ad - bc = \det\begin{bmatrix}a&b\\c&d\end{bmatrix}\)。只有交换两行（变号）和对一行乘以 \(c\)（乘 \(c\)）才会改变行列式值。
>
> **(5) 对。** \(\det(A - \lambda I)=0 \iff A - \lambda I\) 奇异 \(\iff\) 存在 \(\mathbf{x} \neq \mathbf{0}\) 使 \((A - \lambda I)\mathbf{x} = \mathbf{0} \iff A\mathbf{x} = \lambda\mathbf{x}\)。这就是特征方程的标准形式。

---

**A6.** 计算矩阵 $A = \begin{bmatrix} 2 & 0 & 1 \\ 0 & 3 & 0 \\ 1 & 0 & 2 \end{bmatrix}$ 的行列式，并解释其几何含义。
...
> \(\det(A) = 9 \neq 0\) → **可逆**。
>
> **几何含义**：\(A\) 的列是 \((2,0,1)^{\mathsf T}\)、\((0,3,0)^{\mathsf T}\)、\((1,0,2)^{\mathsf T}\)。\(A\) 把 \(\mathbb{R}^3\) 中的单位立方体变换为一个平行六面体，体积 \(= |\det(A)| = 9\)。具体来说：第二列沿 \(y\) 轴拉伸 \(3\) 倍；第一列和第三列在 \(xz\) 平面内张成一个面积为 \(\det\begin{bmatrix}2&1\\1&2\end{bmatrix} = 3\) 的平行四边形；总体积 \(= 3 \times 3 = 9\)。

---

**A7.** 对称矩阵 $B = \begin{bmatrix}2&2&0\\2&5&0\\0&0&3\end{bmatrix}$。

(1) 求 $B$ 的所有特征值。
(2) 求一组标准正交的特征向量，写出谱分解 $B = Q\Lambda Q^{\mathsf T}$。
(3) 利用特征值判断 $B$ 的正定性。
...
**B1.** 某深度网络的 Hessian 特征值为 $\{125,\; 3.2,\; 0.01,\; -0.5,\; -18\}$。判断当前点在优化地形中的位置，预测 SGD 和 Adam 的表现差异。

> **标准答案：**
> 同时存在正特征值和负特征值 → 当前点是**鞍点**（saddle point），而非局部极小。正惯性指数 \(p = 3\)（\(125, 3.2, 0.01\)），负惯性指数 \(q = 2\)（\(-0.5, -18\)）。
> 条件数（对正特征值部分）：\(\kappa = 125 / 0.01 = 12500\)（极差）。
> **SGD 的表现**：
> - 沿 \(\lambda = 125\) 的方向：曲率极大 → 步长稍大即剧烈震荡（overshoot）
> - 沿 \(\lambda = 0.01\) 的方向：几乎平坦 → 寸步难行（每步进展约 \(1/12500\) 的相对量）
> - 沿 \(\lambda = -18\) 的方向：**负曲率** → 梯度下降实际上在沿此方向「逃离」鞍点（因为负曲率方向上的梯度指向下山方向）
> - 典型现象：loss 在鞍点附近「卡住」很久（等梯度噪声将参数推到负曲率方向），然后突然继续下降
> **Adam 的表现**：
> - 自适应学习率处理各方向的不同尺度：\(\lambda=125\) 方向用小步（\(\sqrt{v_t}\) 大）→ 不震荡；\(\lambda=0.01\) 方向用大步（\(\sqrt{v_t}\) 小）→ 加速前进；\(\lambda=-18\) 方向也以合适步长下行 → 快速逃离鞍点
> - 所有 \(5\) 个方向近乎同步收敛，远优于 SGD
...
**B3.** 为什么协方差矩阵必须是半正定的？如果样本协方差矩阵出现负特征值（浮点误差），会有什么实际影响？如何修复？
...
> - **Cholesky 分解失败**：\(\Sigma = LL^{\mathsf T}\) 要求正定；若 \(\Sigma\) 不定 → `np.linalg.cholesky` 抛出 `LinAlgError`
...
**B4.** 设 $A$ 的 SVD 为 $A = U\Sigma V^{\mathsf T}$，其中 $\Sigma = \text{diag}(5, 2, 0)$。

(1) $A$ 的秩是多少？
(2) 描述 $A$ 对 $\mathbb{R}^3$ 中单位球的几何变换（三步拆解）。
(3) $\mathbf{v}_3$ 对应 $\sigma_3=0$。$A\mathbf{v}_3 = ?$ $\mathbf{v}_3$ 属于什么子空间？
(4) （ML）PCA 中若出现很多接近 $0$ 的奇异值，说明什么？

> **标准答案：**
>
> **(1)** 秩 \(=\) 非零奇异值个数 \(= 2\)。零奇异值对应被完全压扁的方向。
>
> **(2)** 三步拆解：
> ① \(V^{\mathsf T}\)：正交矩阵 → 旋转/反射单位球（形状不变，仍是单位球）。
> ② \(\Sigma\)：沿三个坐标轴各自缩放 \(\sigma_i\) 倍。\(\sigma_1=5\) → \(x\) 方向拉伸 \(5\times\)；\(\sigma_2=2\) → \(y\) 方向拉伸 \(2\times\)；\(\sigma_3=0\) → \(z\) 方向压扁到 \(0\)。球变成一个位于 \(xy\) 平面内的椭圆盘。
> ③ \(U\)：再旋转/反射这个椭圆盘，不改变形状。
...
> - 输入维度假高 → 可以大幅降维而不损失信息
> - 模型容量不必正比于名义维度 → **LoRA** 的核心洞察：权重更新做低秩约束，因为问题本身就是低秩的
> - 若强行在高维空间做最小二乘回归 → 极小奇异值在伪逆中变成极大值（\(1/\sigma\)）→ 系数方差爆炸 → 需要正则化

---

**B5.** 设 $A$ 是 $n \times n$ 矩阵，特征值为 $\lambda_1, \dots, \lambda_n$。

(1) 证明 $\det(A) = \prod_{i=1}^n \lambda_i$ 和 $\text{tr}(A) = \sum_{i=1}^n \lambda_i$。
(2) 推论：$\det(A) = 0$ 暗示了什么？
(3) （ML）协方差矩阵的 $\sum \lambda_i$（总方差）和 $\prod \lambda_i$（广义方差）各有什么意义？

> **标准答案：**
>
> **(1)** 假设 \(A\) 可对角化：\(A = PDP^{-1}\)。
> 行列式：\(\det(A) = \det(PDP^{-1}) = \det(P)\det(D)\det(P^{-1}) = \det(D) = \prod \lambda_i\)。
> 迹：\(\text{tr}(A) = \text{tr}(PDP^{-1}) = \text{tr}(P^{-1}PD) = \text{tr}(D) = \sum \lambda_i\)。
> （即使 \(A\) 不可对角化，这两个等式对特征值（含代数重数）仍然成立，通过 Jordan 标准型可证。）
>
> **(2)** \(\det(A) = 0 \iff\) 至少一个 \(\lambda_i = 0 \iff A\) 奇异（不可逆）\(\iff\) 秩亏。
>
> **(3)**
> - **总方差** \(= \sum \lambda_i = \text{tr}(\Sigma)\)：衡量数据在所有方向上的总散布量。PCA 中常看「前 \(k\) 个特征值之和占比」来判断降维保留的信息量。
> - **广义方差** \(= \prod \lambda_i = \det(\Sigma)\)：衡量数据占据的「体积」。\(\det(\Sigma) \approx 0\) 意味着至少一个特征值极小 → 数据几乎分布在一个低于 \(n\) 维的子空间内 → 特征之间存在高度共线性。

---

**B6.** **Rayleigh 商** $R(\mathbf{x}) = \frac{\mathbf{x}^{\mathsf T} H \mathbf{x}}{\mathbf{x}^{\mathsf T} \mathbf{x}}$，$H$ 对称。

(1) $H = \begin{bmatrix} 3 & 1 \\ 1 & 3 \end{bmatrix}$，计算 $R(1,1)$ 和 $R(1,-1)$。
(2) 它们和 $H$ 的特征值有何关系？
(3) （ML）沿负梯度方向 $\mathbf{d} = -\nabla L$，Rayleigh 商如何决定 SGD 的收敛速度？
...
> Hessian 的**条件数** \(\kappa = \lambda_{\max}/\lambda_{\min}\) 表征了最不利和最有利方向之间 Rayleigh 商的比值——它直接决定了 SGD 在最坏情况下的收敛速度。

---

[← 返回教程](determinant-eigen-svd.md)　　　[下一章习题 →](basis-and-neural-networks-exercises.md)