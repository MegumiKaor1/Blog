...
- **Jordan 标准形与矩阵函数**：丘维声《高等代数》（下册）。深入理解不可对角化矩阵的最简形式，以及矩阵指数 $e^A$ 在神经微分方程（Neural ODE）中的角色。
- **中文教材拓展阅读**：工程向——同济《线性代数》（覆盖面广）；数学向——北大《高等代数》（严谨深入）；几何直觉——李尚志《线性代数》；百科全书——丘维声《高等代数》（上下册，适合查阅）。
